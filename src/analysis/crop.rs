...
impl CropDetector {
    ...
    pub async fn detect_crop_values<P: AsRef<Path>>(
        ...
        info!(
            "Starting crop detection analysis with {} sample points",
    ...
    async fn detect_crop_at_timestamp<P: AsRef<Path>>(
        ...
        debug!(
            "Detecting crop at timestamp {:.2}s with limit {} (HDR: {})",
    ...
    fn analyze_crop_frequency(
        ...
        let (most_common_crop, frequency) = crop_frequency
            .iter()
        ...
        debug!(
            "Crop frequency analysis: {:?} appears {}% of the time, {:.2}% pixel change",
    ...
    pub async fn validate_crop_detection(
        ...
        let matching_results = validation_results
            .iter()
            .filter(|result| {
        ...
        debug!(
            "Crop validation: {}/{} samples match, confidence: {:.1}%",
            matching_results,
        ...
        match sample_point {
            ...
            point if point.ends_with('s') => {
                ...
                if let Some(negative_time) = time_str.strip_prefix('-') {
                    let offset: f64 = negative_time.parse().map_err(|_| {
                        crate::utils::Error::validation("Invalid negative time format")
                    })?;
                    Ok(duration - offset)
                } else {
                    time_str
...
mod tests {
    ...
    fn test_sample_point_parsing() {
        ...
        assert_eq!(
            detector.parse_sample_point("middle", duration).unwrap(),
            1800.0
        );
        assert_eq!(detector.parse_sample_point("60s", duration).unwrap(), 60.0);
        assert_eq!(
            detector.parse_sample_point("-60s", duration).unwrap(),
            3540.0
        );
