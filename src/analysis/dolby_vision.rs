...
impl DolbyVisionDetector {
    ...
    fn parse_dolby_vision_metadata(&self, json: &serde_json::Value) -> Result<DolbyVisionInfo> {
        ...
        debug!(
            "Stream info - codec: {}, profile: {}, tag: {}",
            codec_name, profile, codec_tag
        );
        ...
        if dv_info.is_dolby_vision() {
            info!(
                "Detected Dolby Vision Profile {}: RPU={}, EL={}",
                dv_info.profile.as_str(),
                dv_info.has_rpu,
                dv_info.has_enhancement_layer
            );
        } else {
    ...
    fn parse_side_data(
        &self,
        dv_info: &mut DolbyVisionInfo,
        side_data: &serde_json::Value,
    ) -> Result<()> {
        ...
        if let Some(dv_profile_num) = side_data["dv_profile"].as_u64() {
            ...
            debug!(
                "Detected Dolby Vision profile {} from numeric value",
                dv_profile_num
            );
        }
        // Fallback to string parsing
            ...
            if let Some(profile) = DolbyVisionProfile::from_string(dv_profile_str) {
                dv_info.profile = profile;
                debug!(
                    "Detected Dolby Vision profile from string: {}",
                    dv_profile_str
                );
            }
        }
        ...
        if let Some(el_present) = side_data["el_present_flag"].as_bool() {
            ...
            dv_info.has_enhancement_layer = el_present;
            debug!(
                "Enhancement Layer present flag (int): {} -> {}",
                el_present_int, el_present
            );
        }
        ...
        if let Some(rpu_present) = side_data["rpu_present_flag"].as_bool() {
            ...
            dv_info.has_rpu = rpu_present;
            debug!(
                "RPU present flag (int): {} -> {}",
                rpu_present_int, rpu_present
            );
        }
...
mod tests {
    ...
    fn test_dolby_vision_profile_conversion() {
        assert_eq!(
            DolbyVisionProfile::from_string("dvhe.05"),
            Some(DolbyVisionProfile::Profile5)
        );
        assert_eq!(
            DolbyVisionProfile::from_string("dvhe.07"),
            Some(DolbyVisionProfile::Profile7)
        );
        assert_eq!(
            DolbyVisionProfile::from_string("dvhe.08.06"),
            Some(DolbyVisionProfile::Profile81)
        );
        assert_eq!(
            DolbyVisionProfile::from_string("8.1"),
            Some(DolbyVisionProfile::Profile81)
        );
