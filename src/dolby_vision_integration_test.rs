...
mod tests {
    ...
    fn test_dolby_vision_detector_creation() {
        ...
        assert_eq!(
            detector.get_target_profile(DolbyVisionProfile::Profile7),
    ...
    fn test_profile_dolby_vision_compatibility() {
        ...
        x265_params.insert(
            "profile".to_string(),
            serde_yaml::Value::String("main10".to_string()),
        );
        x265_params.insert(
            "pix_fmt".to_string(),
            serde_yaml::Value::String("yuv420p10le".to_string()),
        );
    ...
    fn test_dolby_vision_x265_parameter_building() {
        ...
        let params_str = profile.build_x265_params_string_with_dolby_vision(
            ...
            Some(
                &"G(0.17,0.797)B(0.131,0.046)R(0.708,0.292)WP(0.3127,0.329)L(1000,0.01)"
                    .to_string(),
            ), // master_display
    ...
    fn test_dolby_vision_profile_conversion_logic() {
        ...
        assert_eq!(
            detector.get_target_profile(DolbyVisionProfile::Profile81),
            DolbyVisionProfile::Profile81
        );
    }
...
pub async fn demo_dolby_vision_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  - DV Compatible: {}",
        profile.is_dolby_vision_compatible()
    );
