...
impl EnhancedProgressTracker {
    ...
    fn check_for_stall(&mut self, now: Instant) -> Result<()> {
        ...
        if time_since_last >= self.stall_threshold {
            ...
            if !frame_progress && !time_progress {
                warn!(
                    "Encoding appears to have stalled - no progress for {:.1} seconds",
                    time_since_last.as_secs_f32()
                );

                // Could implement stall recovery here
                debug!(
                    "Stall details - Current: frame={}, time={:.2}s | Last: frame={}, time={:.2}s",
                    self.metrics.current_frame,
    ...
    pub fn finish_successfully(&self) {
        ...
        info!(
            "Encoding completed successfully in {:.1} seconds",
            elapsed.as_secs_f32()
        );
    }
...
mod tests {
    ...
    fn test_stall_detection() {
        let tracker =
            EnhancedProgressTracker::new(3600.0, 30.0, "Test", true, 15, 1000, true, true);
