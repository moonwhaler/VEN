...
impl FfmpegWrapper {
    ...
    pub fn parse_progress_line(&self, line: &str, total_duration: f64) -> Option<ProgressInfo> {
        ...
        if let Some(captures) = PROGRESS_REGEX.captures(line) {
            ...
            progress.time =
                (hours * 3600 + minutes * 60 + seconds) as f64 + centiseconds as f64 / 100.0;
    ...
    async fn parse_video_metadata(
        ...
        let fps = self
            .parse_fraction_to_float(fps_str)
    ...
    fn extract_duration_with_regex(&self, input_path: &str) -> Result<f64> {
        ...
        if let Some(duration_line) = text_output
            .lines()
...
mod tests {
    ...
    fn test_detect_hdr() {
        ...
        assert!(!ffmpeg.detect_hdr(&Some("bt709".to_string()), &Some("bt709".to_string())));
